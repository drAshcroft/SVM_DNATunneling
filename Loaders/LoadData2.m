function [experiment_Index,analyteList,runParams]=SaveFolders(folderPaths, runParams, redoFileload, conn)
...
% sql ='delete from analytes;';
% ret=exec(conn,sql);
% 
% sql = 'delete from analytefolders;';
% ret=exec(conn,sql);
...
for I=1:size(folderPaths,1)
    ...
    if strcmp(folderPaths{I,4},'test')
        sql =[sql  num2str(folderPaths{I,7}) ',' num2str( folderPaths{I,6}) ',2);'];
    else
        
        if strcmp(folderPaths{I,4},'mix')
            sql =[sql  num2str(folderPaths{I,7}) ',' num2str( folderPaths{I,6}) ',3);'];
        else
            sql =[sql  num2str(folderPaths{I,7}) ',' num2str( folderPaths{I,6}) ',0);'];
        end
    end
    exec(conn,sql);
end
