    ...
    for J=1:length(files)
        ...
        if isempty(findstr(fn,'Rinse'))==false || (K==1) %#ok<FSTR>
           ...
           % shortData = restore_imageDPGMM(shortData',covar ,21,100, .8, 5)';
            ...
            shortData = shortData(floor(.4*length(shortData)):floor(.9*length(shortData)));
            sD=shortData(1:5000);
            covar=std(sD);
            ...
            [cShortData, levels, classed] = restore_imageDPGMM(shortData',covar ,9,100, .8, 5);
            ...
            save([p '\'  fn '_dpgmm.mat'],'shortData','cShortData','levels','classed');
