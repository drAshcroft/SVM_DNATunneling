        ...
        if isempty(findstr(fn,'Rinse'))==false || (K==1) %#ok<FSTR>
           ...
           % shortData = restore_imageDPGMM(shortData',covar ,21,100, .8, 5)';
            ...
            for chunks =1:stepSize:length(shortData)
                ...
                [tShortData, cShortData, levels, classed] = restore_imageVGMM(ttt',covar ,9,100, .8, 6);
            ...
            save([p '\'  fn '_Vgmm2.mat'],'shortData','cShortData','nShortData','levels','classed','peakValues','peakValuesM');
