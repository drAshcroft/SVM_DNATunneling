    ...
    for J=1:length(files)
        ...
        if isempty(findstr(fn,'Rinse'))==false || (K==1) %#ok<FSTR>
           ...
           % shortData = restore_imageDPGMM(shortData',covar ,21,100, .8, 5)';
            ...
            shortData = shortData(floor(.25*length(shortData)):floor(.5*length(shortData)));
            sD=shortData(1:5000);
            covar=std(sD);
            ...
            for chunks =1:stepSize:length(shortData)
                ...
                [tShortData, cShortData, levels, classed] = restore_imageVGMM3(ttt',covar ,15,100, .8, 15);
            ...
            save([p '\'  fn '_Vgmm6.mat'],'shortData','cShortData','nShortData','levels','classed','peakValues','peakValuesM','bottom','widths');
