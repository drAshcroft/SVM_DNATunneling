...
colors={ ControlCol ACol GCol CCol ACol GCol CCol TCol mCCol };
analyteNames = {'Control', 'dAMP', 'dGMP', 'dCMP','aTMP','mCMP','peptide','ABasic'};
analyteMap = [mControl mA mG mC mA  mG mC mT mmC];
...
props={};
x=1;
for K=2:min([ size(FileDatas,1) length(pathnames) 5])
    ...
    for J=1:length(files)
        ...
        if isempty(findstr(fn,'ref_n100mv'))==false
            if isempty(findstr(fn,'p380mv'))==false || (plotMode==voltageMode)
                ...
                if isempty(findstr(fn,'1nm'))==false
                    doPlot=true;
                    concentration = '1nm';
            ...
            if isempty(shortData)==false
               ...
               % shortData = shortData - feval(f1,1:length(shortData))';%+ feval(f1,1);
                ...
                if isRinse==false
                    ...
                    r2=(r2'- feval(f1,1:length(shortData)))-noiseSize;
                    %shortData=bShortData';
                    figure(1);clf;
                    plot(bShortData);hold all;
                    bShortData = smooth(bShortData,35);
                    plot(bShortData);
                    peakThresh=max([35 noiseSize*5]);
                    tallIDX = (bShortData>peakThresh);
                    figure(2);
                    plot(tallIDX);hold all;
                    
                    tallIDX=(smooth(tallIDX,65)>0);
                    plot(tallIDX);hold off;
                    figure(1);
                    
                    idx=find(tallIDX>0);
                    X=1:length(bShortData);
                    plot(X(idx),bShortData(idx));
                    ...
                    for I=2:length(allStarts)-2
                        ...
                        seg2=r2(allStarts(I):allEnds(I));
                        dispTrace( allStarts(I):allEnds(I))=1;
                        amp(I-1) =max(seg2)-min(seg);
                        life(I-1)=length(seg);
                        try
                            mn = mean([ mean(seg) min(seg)]);
                            jump(I-1)=abs(mean(seg(seg<mn))-max(seg(seg>mn)));%abs(min(seg)-shortData(allStarts(I)+2));
                        catch mex
                            jump(I-1)=0;
                        end
                    end
                    
                    peakTrace=dispTrace.* bShortData;
                    
                     plot(peakTrace);hold off;
...
if true 
    %save('c:\data\\20140402_IBM_A_11_RIE_AGCTmC_amplitudehistogram_0409.mat','saveAble');
end
