    ...
    for J=1:length(files)
        ...
        if isempty(findstr(fn,'Rinse'))==false || (K==1) %#ok<FSTR>
           ...
           % shortData = restore_imageDPGMM(shortData',covar ,21,100, .8, 5)';
            ...
            shortData = shortData(floor(.25*length(shortData)):floor(.98*length(shortData)));
            sD=shortData(1:5000);
            covar=std(sD);
            %  ttt=shortData(1:500000)';
            %             stepSize=100000;
            %             lShortData=[];
            %             for chunks =1:stepSize:length(shortData)-stepSize
            %                 ttt= shortData(chunks:chunks+stepSize);
            [cShortData, levels, classed] = restore_imageVGMM(shortData',covar ,9,100, .8, 15);
            %lShortData=[lShortData bShortData]; %#ok<AGROW>
            %             end
            ...
            save([p '\'  fn '_Vgmm.mat'],'shortData','cShortData','levels','classed','peakValues','peakValuesM');
