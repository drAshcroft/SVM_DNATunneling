...
masterPaths{5}='S:\Research\BrianAnalysis\Stacked Junctions\20140512_IBM_AA_02_RIE_AGC_10nM\Files for Brian';
for asdf=5:length(masterPaths)
    mI=asdf;
        ...
        for J=1:length(files)
            ...
            if isempty(findstr(fn,'Rinse'))==false || (K==1) %#ok<FSTR>
                ...
                for chunks =1:stepSize:length(shortData)
                    ...
                    [tShortData, cShortData, levels, classed] = restore_imageVGMM3(ttt',covar ,21,100, 10, 10);
...
%                 wShortData = cwt(shortData,levels,'sym2','plot');
                ...
                save([p '\'  fn '_Vgmm13.mat'],'fn','shortData','cShortData','nShortData','levels','classed','peakValues','peakValuesM','bottom','widths');
