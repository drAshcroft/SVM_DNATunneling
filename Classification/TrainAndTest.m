function [accur]=TrainAndTest(experiment_Index,parameterSet_Index,conn,analyteNames, colNames, reducedData,runParams, SVMParams)
...
%Plot_PCA(reducedData,runParams,parameterSet_Index);

if runParams.Order_by_Random
    try
       accur= TestOnRandom(conn,analyteNames,analytes,reducedData, SVMParams, runParams, colNames,parameterSet_Index);
    catch mex
        fprintf([mex.message '\n']);
        for I=1:length(mex.stack)
            try
